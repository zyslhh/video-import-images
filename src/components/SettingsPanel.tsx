...
import type { ExportSettings, ImageFormat } from "../types";
import {
  Settings,
  Image,
  FileType,
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            {(["image/jpeg", "image/webp", "image/png"] as ImageFormat[]).map(
              (fmt) => (
                <button
                  key={fmt}
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            onChange={(e) =>
              handleChange("quality", parseFloat(e.target.value))
            }
            className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-primary"
          />
          <p className="text-xs text-slate-500">{t.qualityHelp}</p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-primary"
          />
          <p className="text-xs text-slate-500">{t.scaleHelp}</p>
        </div>
