...
export const AutoCaptureModal: React.FC<AutoCaptureModalProps> = ({
  ...
  const estimatedScans =
    interval > 0 ? Math.floor((endTime - startTime) / interval) + 1 : 0;
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  {detectedFps ? (
                    <span className="text-[10px] text-blue-300/60 font-normal">
                      ({detectedFps} {t.fpsDetected})
                    </span>
                  ) : (
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      value={endTime}
                      onChange={(e) =>
                        setEndTime(Math.min(duration, Number(e.target.value)))
                      }
                      className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-primary outline-none transition-all"
                    />
                    <p className="text-xs text-slate-500 font-mono text-right">
                ...
                <div className="space-y-2">
                  <div className="flex gap-4 mb-2">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      ...
                      <span
                        className={`text-xs font-semibold uppercase tracking-wider ${
                          mode === "fps" ? "text-primary" : "text-slate-400"
                        }`}
                      >
                        {t.byFps}
                      </span>
                  ...
                  {mode === "interval" ? (
                    ...
                    <div className="relative">
                      <input
                        ...
                        value={fps}
                        onChange={(e) =>
                          handleFpsChange(Math.max(0.1, Number(e.target.value)))
                        }
                        className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-primary outline-none transition-all"
                      />
                      <div className="absolute right-3 top-2.5 text-xs text-slate-500">
                  ...
                  <p className="text-xs text-slate-500">
                    {mode === "interval"
                      ? `${t.intervalVal}: ${interval}s`
                      : `${t.extractingVal} ${fps} FPS`}
                  </p>
                </div>
